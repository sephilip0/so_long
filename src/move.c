...
void	color_screen(t_var *data, int color)
{
	...
	cur_y = 0;
	while (cur_y < SIDE_LEN)
	{
		cur_x = 0;
		while (cur_x < SIDE_LEN)
		{
...
void	color_triangle(int cur_x, int cur_y, t_var *data, int color)
{
	int	center;
//	int	cur_x;
//	int	cur_y;
	...
	center = SIDE_LEN / 2;
	cur_y = center - height;
// isosceles triange built from center upwards
	while (cur_y < center)
	{
		cur_x = center - base;
		while (cur_x < (center + base))
		{
			//working on a square window
			if (((cur_x - center) > ((base * cur_y / height) - base)) && ((cur_x - center) < ((-1 * base * cur_y / height) + base)))
				my_pixel_put(&data->img, cur_x, cur_y, color);
			cur_x++;
		}
		cur_y++;
...
void	color_circle(int cen_x, int cen_y, t_var *data, int color)
{
	int	cur_x;
	int	cur_y;
	int	aux;
...
//circle seems a bit trimmed, no?
	printf("circle cen_x: %d cen_y: %d\n", cen_x, cen_y);
	radius = 50;
	cur_y = cen_y - radius;
	while (cur_y < (cen_y + radius))
	{
		cur_x = cen_x - radius;
		while (cur_x < (cen_x + radius))
		{
			//working on a square window
			aux = (cur_x - cen_x) * (cur_x - cen_x) + (cur_y - cen_y) * (cur_y - cen_y);  
	...
	my_pixel_put(&data->img, cen_x, cen_y, 16711680);
	printf("fim de circle cur_x: %d cur_y: %d\n", cur_x, cur_y);
	my_pixel_put(&data->img, cur_x, cur_y, 32768);
}
/*
void	color_square(int cur_x, int cur_y, t_var *data, int color)
{
	int	center;
//	int	cur_x;
//	int	cur_y;
	...
	center = SIDE_LEN / 2;
	cur_y = center - (Y / 2);
	while (cur_y < (center + (Y / 2)))
	{
		cur_x = center - (X / 2);
		while (cur_x < (center + (X / 2)))
		{
			my_pixel_put(&data->img, cur_x, cur_y, color);
			cur_x++;
		}
...
int	f(int keysym, t_var *data)
{
	...
	{
		what_y -= step;
		color_circle(what_x, what_y, data, 0xffff);
	}
	else if (keysym == XK_s)
	{
		what_y += step;
	...
	{
		color_square(what_x, what_y, data, 0xff0000);
	//	color_screen(data, 0xff0000);
	}
	else if (keysym == XK_t)
	{
		color_triangle(what_x, what_y, data, 0xffffff);
	...
	{
		color_square(what_x, what_y, data, 0xff00);
		//color_screen(data, 0xff00);
	}*/
	else if (keysym == XK_b)
	{
