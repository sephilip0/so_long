...
void	color_triangle(t_var *data, int color)
{
	int	center;
	int	cur_x;
	int	cur_y;
...
void	color_circle(t_var *data, int color)
{
	int	center;
	int	cur_x;
	int	cur_y;
...
//circle seems a bit trimmed, no?
	...
	center = SIDE_LEN / 2;
	cur_y = center - radius;
	while (cur_y < (center + radius))
	{
		cur_x = center - radius;
		while (cur_x < (center + radius))
		{
			//working on a square window
			aux = (cur_x - center) * (cur_x - center) + (cur_y - center) * (cur_y - center);  
...
void	color_square(t_var *data, int color)
{
	int	center;
	int	cur_x;
	int	cur_y;
...
int	f(int keysym, t_var *data)
{
	...
	if (keysym == XK_r)
	{
		color_square(data, 0xff0000);
	//	color_screen(data, 0xff0000);
	}
	else if (keysym == XK_t)
	{
		color_triangle(data, 0xffffff);
		//color_screen(data, 0xff00);
	}
	else if (keysym == XK_c)
	{
		color_circle(data, 0xffff);
		//color_screen(data, 0xff00);
	}
	else if (keysym == XK_g)
	{
		color_square(data, 0xff00);
		//color_screen(data, 0xff00);
	}
	else if (keysym == XK_b)
	{
